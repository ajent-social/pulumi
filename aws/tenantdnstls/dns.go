...
//
// Apex + wildcard under the same parent share one ACM DNS validation CNAME.
// This component creates that record at registration time, passes every
// DomainValidationOption name into CertificateValidation, and DependsOn the
// Route53 record so validation cannot race ahead of DNS.
func New(ctx *pulumi.Context, name string, args Args, opts ...pulumi.ResourceOption) (*Bundle, error) {
	...
	// Include every option's validation name (apex + wildcard). For this SAN
	// shape AWS returns the same CNAME; listing both keeps CertificateValidation
	// complete if options are reported separately.
	validationFQDNs := cert.DomainValidationOptions.ApplyT(func(opts []acm.CertificateDomainValidationOption) ([]string, error) {
		var fqdns []string
		for _, opt := range opts {
			if opt.ResourceRecordName == nil {
				continue
			}
			fqdns = append(fqdns, *opt.ResourceRecordName)
		}
		if len(fqdns) == 0 {
	...
	}, pulumi.Parent(component), pulumi.DependsOn([]pulumi.Resource{validationRecord}))
	if err != nil {
